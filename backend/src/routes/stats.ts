...
router.get('/', async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        for (const habit of habits) {
            ...
            bestCurrentStreak = Math.max(bestCurrentStreak, calculateCurrentStreak(dates));
            bestLongestStreak = Math.max(bestLongestStreak, calculateLongestStreak(dates));
        }
...
router.get('/habit/:id', async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const completionRecords = await prisma.habitCompletion.findMany({
            where: { habitId: id },
            select: { date: true },
            orderBy: { date: 'asc' },
        });

        const dates: string[] = completionRecords.map((c) => c.date);
...
router.get('/analytics', async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const progression = await Promise.all(Array.from({ length: 14 }, (_, i) => {
            ...
            const completed = await prisma.habitCompletion.count({
...
router.get('/weekly-review', async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        const userId = req.userId!;
        ...
        const completionsByHabit: Record<string, any[]> = {};
...
async function buildWeeklyData(userId: string, totalHabits: number) {
    const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
    const today = new Date();
...
async function buildHeatmap(userId: string, totalHabits: number) {
    const today = new Date();
